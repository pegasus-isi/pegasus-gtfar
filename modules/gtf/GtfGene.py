...
class GtfGene:
    ...
    def getSeqFromChr(self,chrSeq,index=1):
       
        if index==1:
            self.seq=['N']
            self.seq.extend(seqCreate(chrSeq[self.start-1:self.end],self.strand))
        else:
            self.seq=seqCreate(chrSeq[self.start-1:self.end],self.strand,index)
